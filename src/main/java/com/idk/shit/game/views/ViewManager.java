...
public class ViewManager {
    public StateManager currentStateManager;
    ...
    public ViewManager(long window, InputManager inputManager, StateManager currentStateManager, long vg, TextRenderer textRenderer, ScoreManager scoreManager ){
        this.window=window;
    ...
    private void UpdateViewByState(){
        ApplicationState state = currentStateManager.GetApplicationState();
        if(state instanceof Menu && !(currentView instanceof MenuView)){
            currentView = new MenuView(currentStateManager, window, inputManager, vg, textRenderer, scoreManager );
        }
        ...
        if(state instanceof GameOver && !(currentView instanceof GameOverView)){
            ...
            currentView = new GameOverView(currentStateManager,window, inputManager, vg, textRenderer, scoreManager);
        }
        if (state instanceof Play && !(currentView instanceof PlayingView)){
            if (state instanceof Level1  && !(currentView instanceof Level1View)){
                currentView=new Level1View(currentStateManager, (Level1) state, window, inputManager, vg, textRenderer, scoreManager);
            }
            if (state instanceof Level2 && !(currentView instanceof Level2View)){
                currentView=new Level2View(currentStateManager, (Level2) state, window, inputManager, vg, textRenderer, scoreManager );
            }
        }
    }
}